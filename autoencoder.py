...
class SSE(nn.Module):
    def __init__(self,config,gen_a=None,gen_p=None):
    ...
    def gen_a_update(self,x_a):
        ...
        # magnitude Decode
        x_a_recon,mag_a_recon = self.gen_a.adecode(h_a+n_a,mag_a)
        # phase decode
        x_p_recon, p_a_recon = self.gen_a.pdecode(h_a + n_a, p_a)
        # Loss
        self.recon_a_loss = self.config['recon_a_w']*self.recon_criterion(mag_a_recon,mag_a)
        self.recon_p_loss = self.config['recon_p_w'] * self.recon_criterion(p_a_recon, p_a)
        self.kl_loss = self.config['kl_a_w']*self.__compute_kl(h_a)

        self.total_loss = self.recon_a_loss + self.recon_p_loss + self.kl_loss
        self.total_loss.backward()
        self.opt_a.step()
        return self.total_loss, x_a_recon, x_p_recon
    ...
    def gen_b_update(self,x_b,notnoise):
        ...
        # amplitude decode (within domain)
        x_b_recon, mag_b_recon = self.gen_b.adecode(h_b+n_b,mag_b)
        # phase decode (within domain)
        x_b_recon, p_b_recon = self.gen_b.pdecode(h_b+n_b,p_b)
        # Decode (cross domain)
        x_ba, mag_ba = self.gen_a.adecode(h_b+n_b,mag_b)
        x_ba1, p_ba = self.gen_a.adecode(h_b + n_b, p_b)
        x_ba = x_ba + x_ba1
        # Encode again
        h_b_recon,n_b_recon,mag_b_recon,p_b_recon = self.gen_a.encode(x_ba)
        # Decode again
        x_bab, mag_bab = self.gen_b.decode(h_b_recon+n_b_recon,mag_b_recon)
        x_bab, p_bab = self.gen_b.decode(h_b_recon + n_b_recon, p_b_recon)
        # Cross domain reconstruction loss for pure noises
        if torch.sum(1-notnoise) != 0:
            self.loss_cross1 = self.config['cross_w']*self.recon_criterion(mag_ba,notnoise=1-notnoise,cross=True)
            self.loss_cross2 = self.config['cross_w'] * self.recon_criterion(p_ba, notnoise=1 - notnoise, cross=True)
            self.loss_cross = self.loss_cross1 + self.loss_cross2
        else:
            self.loss_cross = 0
        # Losses
        self.loss_recon_b1 = self.config['recon_b_w']*self.recon_criterion(mag_b_recon,mag_b)
        self.loss_recon_b2 = self.config['recon_b_w'] * self.recon_criterion(p_b_recon, p_b)
        self.loss_recon_b = self.loss_recon_b1 + self.loss_recon_b2
        self.loss_kl_b = self.config['kl_b_w']*self.__compute_kl(h_b)
        self.loss_recon_cyc_b1= self.config['cyc_recon_b_w']*self.recon_criterion(mag_bab,mag_b,notnoise=notnoise)
        self.loss_recon_cyc_b2 = self.config['cyc_recon_b_w'] * self.recon_criterion(p_bab, p_b, notnoise=notnoise)
        self.loss_recon_cyc_b = self.loss_recon_cyc_b1 + self.loss_recon_cyc_b2
    ...
    def evaluate(self,x_b):
        ...
        x_b_recon, mag_b_recon = self.gen_b.adecode(h_b+n_b,p_b)
        x_b_recon, p_b_recon = self.gen_b.pdecode(h_b + n_b, p_b)
        # Decode (cross domain)
        ...
        h_b_recon,n_b_recon,mag_b_recon,p_b_recon = self.gen_a.encode(x_ba)
        # Decode again
        x_bab1, mag_bab = self.gen_b.decode(h_b_recon+n_b_recon,mag_b_recon)
        x_bab2, p_bab = self.gen_b.decode(h_b_recon + n_b_recon, p_b_recon)
        x_bab = x_bab1 + x_bab2
        self.train()
        return x_b_recon,mag_ba,x_bab,mag_b
...
class VAE(nn.Module):
    ...
    def adecode(self,z,a):
        x, a = self.dec(z,a)
        return x,a

    def pdecode(self,z,p):
        p = self.tconv(p)
        p = (p + EPS) ** (1.0 / 0.5)
        x = self.istft(0, p)
        return x,p
    ...
    def forward(self,x):
        z,a,p = self.enc(x)
        noise = Variable(torch.randn(z.size())).cuda(z.data.get_device())
        x_recon, a, p = self.dec(z+noise,a,p)
        return x_recon, z
